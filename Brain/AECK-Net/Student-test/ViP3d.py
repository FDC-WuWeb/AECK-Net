...
class WeightedPermuteMLP(nn.Module):
    def __init__(self,dim,h_dim,w_dim,d_dim, seg_dim=8, qkv_bias=False, proj_drop=0.,res=False):
        ...
        # self.mlp_h=nn.Linear(dim*5//2,dim*5//2,bias=qkv_bias)
        # self.mlp_w=nn.Linear(dim*7//4,dim*7//4,bias=qkv_bias)
...
if __name__ == '__main__':
    # input=torch.randn(1,128,40,28,16)
    # input=torch.randn(1,128,16,16,16)
    input=torch.randn(1,64,40,28,24)
    # input=torch.randn(1,16,64,40,48)

    vip=WeightedPermuteMLP(64,seg_dim=4,res=True)
    out=vip(input)
    print(out.shape)
