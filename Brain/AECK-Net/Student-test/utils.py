...
def test(modelCAD, modelORG, modelENH,test_loader):
    ...
    for movCAD, refCAD, movORG, refORG, movENH, refENH in test_loader:
        ...
        with torch.no_grad():
            warpedCAD_ORG, warpedCADKD, flowCAD, flow_CADdownKD, flow_CADoutKD = modelCAD(movCAD, refCAD, movORG)
            warpedORG_ENH, warpedORGKD, flowORG, flow_ORGdownKD, flow_ORGoutKD = modelORG(warpedCAD_ORG, refORG,
                                                                                          flowCAD, movENH)
            warpedENHKD, flowENH, flow_ENHdownKD, flow_ENHoutKD = modelENH(warpedORG_ENH, refENH, flowORG, movENH)

            Dice = dice_coef(warpedENHKD, refENH)
